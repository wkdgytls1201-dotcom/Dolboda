...
### 10. 저장 검색조건 리마인드 (2026-08-04 구현 완료)

리텐션 강화용으로 추가. 관심 지역보다 훨씬 좁게 타겟팅되는 알림 — `/search`
필터 시트에서 조건을 저장하면(`SavedSearch` 모델, 최대 5개/인), 그 조건에
맞는 새 시설이 등록될 때 알려준다.

- **매칭 범위를 의도적으로 줄였다** — 유형·등급·진료과목·프로그램태그·빈자리·
  공공데이터확인만 매칭한다. `maxDistanceKm`(사용자 위치를 서버에 안 둔다)과
  `goodScoreOnly`(안심지수는 즉석 계산이라 순수 node 스크립트에서 값싸게 재현
  못 한다 — §9의 근무환경지수와 같은 문제를 여기서도 만났다)는 저장은 해두되
  매칭에서는 건너뛴다. 놓치는 것보다 조금 넓게 잡는 쪽을 택했다.
- **클라이언트 컴포넌트에서 타입만 분리** — `FacilityFilters` 타입·`EMPTY_FILTERS`를
  `components/FilterBar.tsx`(클라이언트 컴포넌트)에서 `lib/facilityFilters.ts`(순수
  타입 파일)로 옮겼다. API 라우트가 클라이언트 컴포넌트 파일에서 값을 import하면
  그 파일 전체("use client" + 아이콘 + AuthModal)가 서버 함수 번들에 딸려 들어간다.
- **메일 폭탄 방지** — 조건이 넓으면(유형 하나만) 대량 등록일에 수천 건이 한꺼번에
  매칭될 수 있다(실측: 대량 등록일 하루에 5,071건). 메일당 카드 20개로 자르고
  "외 N건 더"를 안내한다. `AlertDelivery`는 잘린 것 포함 전부 기록해 내일 다시
  안 온다.
- **검증**: 실계정으로 "요양원" 조건 저장 → 저장·목록·삭제 전부 실계정에서 확인.
  발송 스크립트는 대량 등록일(2026-07-30)로 드라이런해 5,071건 매칭·수신자 1명
  정상 계산 확인(`--write`는 안 돌려 실제 발송·DB 기록은 안 만듦).

### 11. 돌봄일지 정기 체크인·등급테스트 재검사 리마인드 (2026-08-04 구현 완료)

리텐션 리스트의 나머지 둘. 둘 다 이 문서의 관심시설/지역 알림과는 대상·수신
경로가 달라 `send-facility-alerts.mjs`에 안 합치고 별도 스크립트·워크플로로
뒀다(대상이 겹치지 않으면 "사람당 메일 한 통" 원칙이 의미가 없다).

- **돌봄일지 체크인** (`scripts/send-care-log-reminders.mjs`) — 진행 중인
  매칭 돌봄의 마지막 일지로부터 3일 이상 지나면 매니저에게. 자세한 설계는
  [docs/care-log-spec.md](care-log-spec.md) §0-2.
- **등급테스트 재검사** (`scripts/send-grade-test-reminders.mjs`) — 처음엔
  새 저장소가 필요할 줄 알았는데, 등급테스트 결과 화면의 "프로필에 저장"을
  타면 이미 `CareProfile.estimatedBand`·`estimatedAt`에 서버 저장되고 있었다
  (`app/api/care-profile/route.ts`). 그 값이 90일 이상 지났고 아직 실제
  장기요양등급(`ltcGrade`)이 없는 프로필에만 보낸다 — 확정 등급이 있으면
  "예상"을 다시 볼 이유가 없다. 쿨다운 30일.

두 스크립트 모두 `CareLogReminder`·`GradeTestReminder`라는 같은 모양의
쿨다운 모델을 쓴다(대상 하나당 한 행, `sentAt`만 계속 갱신 — `AlertDelivery`처럼
날짜별로 안 쌓는다. "오늘 보냈나"가 아니라 "최근 며칠 안에 보냈나"만 알면
되는 쿨다운이라 더 단순하다).

**검증의 한계**: 둘 다 대상이 될 실제 데이터가 없다(매칭확정 0건, 예상 구간을
90일 전에 저장해둔 사람 0명). 임시 테스트 계정(`.invalid` 이메일)으로 조건에
맞는 행을 하나씩 만들어 탐지·제외 로직만 확인하고 즉시 삭제했다 — 실제 이메일
발송(`--write`)까지는 검증하지 않았다.

### 12. 매니저 알림 인프라 서버 이전 + 새 일자리 알림 (2026-08-04 구현 완료)

§11 끝에 "먼저 해야 한다"고 적어뒀던 것을 같은 세션에서 이어서 처리했다.

**서버 이전** — `/mypage/sitter/notifications`가 `localStorage`뿐이라 서버가
"누가 알림을 원하는지" 몰랐던 문제. `SitterNotificationPref` 모델(사람당 한 행,
`userId`를 그대로 기본키로 씀) + `/api/sitter/notification-prefs`(GET·PATCH)로
보호자 쪽 찜 알림과 같은 패턴으로 옮겼다. 클라이언트는 `res.ok`를 직접 확인해
저장 실패 시 롤백한다(§1-B "전남"·"광주" 버그의 교훈을 처음부터 반영).

**새 일자리 알림** (`scripts/send-sitter-job-alerts.mjs`) — 오늘 새로 등록된
`CareRequest`(상태 OPEN) 중 매니저의 활동 지역(`SitterProfile.regions`)과
겹치는 걸 찾아, `SitterNotificationPref.newJob`이 켜진 사람에게 보낸다.

- **관심 지역 알림보다 훨씬 간단했다** — `CareRequest.region`과
  `SitterProfile.regions`가 둘 다 `lib/regions.ts`의 `REGIONS`(17개) 표기를
  그대로 쓴다. 시설 알림처럼 주소를 파싱해 지역을 추론할 필요가 없어서
  §9·§10에서 반복됐던 "지역 taxonomy 이중화" 문제 자체가 안 생겼다.
- **쿨다운이 필요 없다** — 공고가 "오늘 새로 생김"은 평생 한 번뿐인 사건이라
  `SitterJobAlertDelivery`(userId+careRequestId 조합을 기본키로 삼아 중복
  발송만 막음, 날짜·쿨다운 개념 없음)로 충분하다.
- **검증**: 실제 프로덕션 데이터로 바로 확인됐다(테스트 계정 안 만들어도
  됨) — 오늘 실제로 등록된 공고 1건이 활동 지역이 겹치는 매니저 3명과
  매칭되고, 그중 이메일이 있는 1명에게 정상적으로 갈 준비가 된 것 확인
  (`--write`는 안 돌려 실제 발송은 안 함).

### 아직 남은 것

- **웹푸시 → 앱 FCM**
- **매칭 진행 알림** (`SitterNotificationPref.matchUpdate`) — 토글은 서버에
  저장되지만 아직 발송 로직이 없다. 다른 알림들과 달리 "매일 새벽 배치"가
  아니라 지원 상태가 바뀌는 그 순간(매칭확정/미선정) 바로 나가야 자연스러운
  성격이라, 배치 스크립트가 아니라 상태를 바꾸는 API 라우트(예:
  `app/api/care-requests/[id]/route.ts`의 매칭확정 처리)에 발송을 직접
  끼워 넣는 다른 모양의 작업이 될 것.
- **근무환경지수 변화 감지 알림** — §1-H(9차 이전 논의)에서 보류. 즉석 계산
  지표라 일별 스냅샷이 없으면 값싸게 못 만든다는 문제는 여전히 안 풀렸다.
