...
- **이메일 없는 계정 안내** — 카카오 로그인은 이메일 제공 동의가 꺼져 있으면 이메일을
  주지 않는다(실측에서 재현됨). `/notifications`에 "알림을 받으려면 이메일이 필요해요"를
  띄워야 한다. 구글 로그인이 붙으면 상당수 해소된다
- **관심 지역 신규 시설 알림** — 찜한 시설이 없는 사용자도 받을 것이 생긴다
- **웹푸시 → 앱 FCM**
