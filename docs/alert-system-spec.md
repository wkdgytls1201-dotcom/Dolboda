...
> 최종 갱신: 2026-08-05 (§3-12 일지 도착·§3-13 요청 취소 추가분까지 반영)
...
1. **배치형** — 매일 아침 9시(한국 시각), 서버가 "지난 하루 사이 무슨 일이
   ...
   같은 시각에 자동 실행시킨다.
2. **즉시형** — 돌봄이 진행되는 **그 순간**(지원이 왔을 때, 매칭이 확정됐을 때,
   완료 처리됐을 때, 돌봄일지가 도착했을 때) 배치를 기다리지 않고 바로 이메일을
   보낸다. Next.js API 라우트 안에 직접 붙어 있다(별도 스크립트·워크플로 없음).
...
| 9 | 돌봄일지 주간 요약(§3-15) | 보호자 | `send-care-log-weekly.mjs` | `send-care-log-weekly.yml` (**월요일만**) |
...
| 12 | 돌봄일지 도착 | 보호자 | `POST /api/care-log` (하루 첫 기록일 때) |
| 13 | 요청 취소 | 매니저(지원완료·매칭확정 전원) | `DELETE /api/care-requests/[id]` |
...
| 이메일 | "보호자님이 어르신 소식을 기다리고 있어요" — **채근·지시 어투 금지**(매니저는 고용 관계가 아니라 관리감독으로 읽히면 사용자성 징표가 된다, care-log-spec §10-5). 보호자의 실제 반응이 있으면 인용하고, 자율 기록 고지를 상시 포함 |
...
|---|---|
| 트리거 | 매일 09:00 KST |
| 데이터 소스 | `CareRequest`(상태 `OPEN`, **어제~오늘 이틀 창**에 새로 생성됨 = 매니저 입장에선 "새 채용 공고"). 창이 이틀인 이유: 크론이 09:00 하루 한 번이라 당일 창만 보면 09:00 이후 올라온 공고가 영영 빠졌다(2026-08-05 실측·수정). 창이 겹쳐도 발송 기록 유니크가 중복을 막는다. `--date` 지정 시엔 그 하루만 |
...
| 쿨다운 | 없음(평생 한 번뿐인 사건 — 이틀 창의 중복은 `SitterJobAlertDelivery`가 차단) |
...
### 3-12. 돌봄일지 도착 (2026-08-05 추가)

| 항목 | 내용 |
|---|---|
| 트리거 | 매니저가 일지를 저장하는 순간(`POST /api/care-log`) — 단, **그 날짜의 첫 기록일 때만**(정정 기록 제외) |
| 수신자 | 그 돌봄 건의 보호자 |
| 발송 조건 스위치 | 없음 — §3-8과 같은 거래성 원칙(보호자 쪽 알림 설정 모델이 생기면 스위치를 붙일 것) |
| 중복방지 | 별도 기록 테이블 없음 — "careDate당 원본 기록 1건" 유니크 가드(409)가 곧 발송 1회 보장이다. 정정(`correctsId`)은 도착 알림을 다시 보내지 않는다 |
| 특이사항과의 관계 | `dayStatus`가 "알려드릴 일이 있어요"면 특이사항 메일(§0의 notifyAlert)이 나가고 도착 알림은 **생략**한다 — 같은 저장에 두 통을 보내지 않는다. 정정 기록의 특이사항은 여전히 특이사항 메일이 나간다 |
| 이메일 | "2026-08-05 돌봄일지가 도착했어요" (매니저 닉네임 + 확인 경로 안내) |
| 왜 즉시형인가 | 보호자가 "오늘 저녁에" 읽는 것이 가치다 — 다음날 아침 배치면 늦다. 일지의 존재 이유가 보호자 확인(읽음 표시)이라, 도착을 알려야 매일 열어보는 순환이 시작된다 |
| ⚠️ 검증 상태 | 매칭확정 실데이터가 없어 §3-5와 같은 사정으로 코드 경로 검토 + 타입 검사만. 발송 로직은 기존 notifyAlert와 동일 구조 |

### 3-13. 요청 취소 (2026-08-05 추가 — §6-5의 후보를 실행)

| 항목 | 내용 |
|---|---|
| 트리거 | 보호자가 요청을 취소하는 순간(`DELETE /api/care-requests/[id]`) |
| 수신자 | 그 요청의 "지원완료"·"매칭확정" 지원자 전원(상태가 "요청취소"로 바뀌는 사람들) |
| 발송 조건 스위치 | `SitterNotificationPref.matchUpdate`(미선정 §3-10과 같은 무게의 소식이라 같은 스위치) |
| 중복방지 | 필요 없음(취소도 평생 한 번 — `OPEN·MATCHED` 상태 가드가 재취소를 막는다) |
...
### 3-15. 돌봄일지 주간 요약 (2026-08-06 추가 — 배치형, 번호는 추가순)

| 항목 | 내용 |
|---|---|
| 트리거 | **월요일 09:00 KST만**(다른 배치는 매일) — 스크립트가 "지난 주 월~일"을 계산하므로 월요일에 돌려야 직전 한 주가 딱 잡힌다 |
| 데이터 소스 | `CareLog`(지난 주 `careDate` 범위) + `CareRequest`(`MATCHED`·`COMPLETED`) |
| 판정 | 지난 주에 기록이 **1건 이상** 있는 돌봄 건. **0건이면 보내지 않는다** — 빈 요약은 매니저에 대한 무언의 지적이 된다 |
| 수신자 | 그 돌봄 건의 보호자(이메일 있는 경우) |
| 쿨다운 | **돌봄 건×주 1회**(`CareLogWeeklyDigest`, 기본키 `[careRequestId, weekOf]`) — 같은 주에 수동 재실행해도 두 번 안 나간다 |
| 이메일 | "지난 주 돌봄 기록을 정리했어요" + 기록한 날 수·식사 잘하신 날·알려드린 일·사진 수 + **그 주 마지막 한마디 인용**(숫자보다 이 한 줄이 크게 남는다) |
| 구현 메모 | 정정이 여러 행일 수 있어 **날짜별 최신 1건만** 센다(화면 셈법과 동일). 발송 전 기록(§2), `--write`인데 키 없으면 기록도 없이 중단(§3-14와 같은 이유) |
| ⚠️ 문구 원칙 | **매니저를 향한 압박 금지**(care-log-spec §10-5 · 매니저 법적 거리 방침) — "며칠 빠졌다"를 지적하거나 독촉을 유도하지 않는다. 있는 것을 그대로 전하고 "기록이 없는 날도 있어요"로 담담히 닫는다 |
...
| `CareLogWeeklyDigest` | §3-15의 발송 기록 | `[careRequestId, weekOf]`가 기본키(돌봄 건×주 1회 — 같은 주 재실행에도 두 번 안 나감) |
...
5. ~~**취소 알림**~~ → 완료(2026-08-05): §3-13으로 구현.
...
scripts/
  ...
  send-care-log-weekly.mjs        # §3-15 (월요일만)
