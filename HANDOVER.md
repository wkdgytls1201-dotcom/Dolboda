...
| 시설 유형 | 개수 | 좌표 확보 | 평가등급 보유 |
|---|---|---|---|
| 방문요양센터 (HOME_CARE) | 12,957 | 12,715 | 6,989 |
| 요양원 (NURSING_HOME) | 5,073 | 4,801 | 3,948 |
| 주야간보호 (DAY_NIGHT_CARE) | 4,509 | 4,037 | 2,588 |
| **요양병원 (NURSING_HOSPITAL)** | **1,303** | 1,303 | 1,231 |
| 실버타운 (SILVER_TOWN) | 1 | 1 | 0 |
| **합계** | **23,843** | 22,857 | 14,756 |

요양병원 1,303곳은 이번에 새로 수집했습니다(기존 21곳 → 1,303곳). 인력등급 1,247곳, 진료과목 1,299곳, 병상수 1,301곳, 의료장비 1,195곳까지 채워져 있습니다.
...
**① 요양시설(NHIS) 지역 데이터 누락 — 인천·전북·경북**
```
인천광역시 57  | 전북특별자치도 72 | 경상북도 102
```
이 숫자는 전부 이번에 넣은 **요양병원**이고, 요양원·주야간보호·방문요양은 **0건**입니다.
- 원인: `scripts/import-nhis-v2.mjs`가 읽는 `C:\Users\linea\Desktop\지역 장기` 엑셀 16개 중 인천·전북·경북 파일이 제대로 파싱되지 않은 것으로 추정 (파일 내용이 뒤섞여 있음 — 강원도.xlsx를 열었더니 제주 데이터가 들어있었음)
- 할 일: 해당 엑셀을 다시 받아 재임포트. 노인장기요양보험 홈페이지(longtermcare.or.kr)에서 지역별 "장기요양기관내역" 다운로드
