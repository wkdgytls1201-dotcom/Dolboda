...
## 12. 참고자료 — 케어닥 스크린샷 위치 (재전송 불필요)

사용자가 공유했던 케어닥 모바일 스크린샷은 전부 바탕화면 폴더에 그대로 있음. 새 세션에서 필요하면 아래 경로를 Read로 직접 열어볼 것. 단, **핵심 결정사항은 이미 이 문서와 계획에 반영돼 있으므로 남은 작업(6번)은 스크린샷 없이도 진행 가능** — 확인서 페이지나 마법사 UI 디테일이 궁금할 때만 참고.

| 폴더 | 내용 |
|---|---|
| `C:\Users\linea\Desktop\1-30`, `31-50`, `51-55` | 보호자 측 간병 신청 흐름 55장 (돌봄 요청 v1의 원본 참고자료) |
| `C:\Users\linea\Desktop\케어닥` | 케어코디(시터) 등록/프로필/일자리 관리 화면들 (모심시터 시스템 원본 참고자료) |
| `C:\Users\linea\Desktop\모바일 필터, 병원` | 병원/가사돌봄/집에서 필터 목록 화면 14장 ← 진행 중 작업 참고 |
| `C:\Users\linea\Desktop\모바일 가사돌봄` | 가사돌봄 타입 신청 흐름 8장 (체크리스트, 방문횟수/시간) ← 진행 중 작업 참고 |
| `C:\Users\linea\Desktop\모바일 집에서` | 집에서(자택) 타입 6장 ← 진행 중 작업 참고 |
| `C:\Users\linea\Desktop\모바일 모심시터 신청서 계약서 PDF 자동생성` | 계약서 자동생성 16장 (우리는 "돌봄 확인서"로 대체) ← 진행 중 작업 참고 |
| `C:\Users\linea\Desktop\지원하기 했을떄 주민번호 필요` | 지원 시 주민번호 요구 8장 (우리는 수집 안 하기로 결정) |

⚠️ 스크린샷은 어디까지나 참고용 — 워딩/구성을 그대로 베끼지 않는다는 규칙(7-4번) 유지.
