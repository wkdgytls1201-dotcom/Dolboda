...
const RULES: { tag: ProgramTag; keywords: string[] }[] = [
  {
    ...
    keywords: [
      ...
      // 감각 자극·교구 활용은 인지 자극이 목적이다.
      // ("오감"은 "오감나무" 같은 프로그램 이름에 그대로 들어가 오탐이 나서 뺐다)
      "교구", "감각조작",
    ],
  },
  {
  ...
  {
    tag: "music",
    keywords: ["음악", "뮤직", "노래", "가요", "합창", "악기", "난타", "장구", "리듬"],
  },
  {
    tag: "art",
  ...
  },
  {
    tag: "family",
    keywords: [
      "가족참여", "가족교육", "가족간담", "보호자간담", "가족상담",
      // 실제 데이터에 흔한 표기들 — 예전엔 "프로그램"이 들어갔다는 이유만으로
      // "(가족대상프로그램) 어버이날"이 여가·교류로 잡혔다
      "가족대상", "가족지지", "가족지원", "가족행사", "어버이날",
    ],
  },
  {
    tag: "spiritual",
  ...
  {
    tag: "outing",
    // "숲"만 넣으면 "꽃숲소리마당" 같은 프로그램 이름이 걸려서 붙여쓴 형태로 좁힌다
    // (공백은 tagOf에서 제거되므로 "숲 체험"도 "숲체험"으로 매칭된다)
    keywords: ["나들이", "산책", "야외", "외출", "소풍", "견학", "바깥", "숲체험", "숲나들이"],
  },
  {
    tag: "grooming",
    keywords: [
  ...
  },
  {
    ...
    keywords: [
      ...
      "다과회", "웃음", "사회적응", "축제", "장터", "극장", "나들이방",
      // ⚠️ 여기에 "프로그램"·"활동" 같은 포괄어를 넣지 말 것.
      // 마지막 규칙이라 앞에서 안 걸린 게 전부 여기로 쏟아진다. 실측(2026-08-01) 결과
      // social 4,914종 중 929종(19%)이 오직 그 두 단어 때문에 "여가·교류"가 돼 있었다
      // ("노인학대예방교육", "(맞춤형) 실버 스포츠", "뮤직플러스" 같은 것들).
      // 보호자가 여가·교류로 거르는 이유는 실제로 그런 활동을 하는 곳을 찾기 위해서다.
      // 못 맞히면 미분류로 두는 게 낫다 — 없는 걸 지어내지 않는다는 원칙과 같다.
      // (원예치료는 garden 규칙이 먼저 "원예"로 잡으므로 여기 적어도 도달하지 않는다)
    ],
  },
];
