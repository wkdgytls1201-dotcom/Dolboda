...
//
// 구간 기준은 임의값이 아니라 실제 분포(2026-08-01, 26,715곳)의 사분위수로 잡았다.
...
// 실제 분포(어르신/요양보호사): p10 2.07 · p25 2.25 · p50 3.40 · p75 6.33 · p90 9.00
// 요양원 법정 배치기준이 요양보호사 1명당 2.3명이라 p25 언저리가 "기준을 지키는 수준"이다.
function workloadScore(ratio: number): number {
  if (ratio <= 2.25) return 95;
  if (ratio <= 3.4) return 80;
  if (ratio <= 6.33) return 55;
  if (ratio <= 9) return 35;
  return 20;
}

// 실제 분포(1년 이상 근속 비율): p25 40.4% · p50 60% · p75 75%
...
export function calcWorkIndex(f: Facility): WorkIndex {
  ...
  const workload: WorkSignal[] = [
    {
      ...
      score: workloadRatio == null ? null : workloadScore(workloadRatio),
      weight: 3,
      note:
        workloadRatio == null
  ...
  if (occupancy !== undefined && occupancy > 0 && careWorkers > 0) {
    ...
    workload.push({
      label: "현재 실제 담당 인원",
      score: workloadScore(actual),
      weight: 2,
      note: `현원 기준 1인당 ${actual}명`,
    });
  }
