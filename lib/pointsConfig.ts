...
/** 포인트 유효기간(개월) — 약관 제12조의2·화면 고지와 같은 값.
 *
 *  2026-08-06 감사에서 **고지만 있고 구현이 없던 것**을 확인해 그날 구현했다
 *  (`lib/points.ts`의 `usableBalance`). 잔액 조회와 사용 차감이 같은 함수를 쓰므로,
 *  "화면엔 있는데 결제는 되는" 어긋남이 생기지 않는다.
 *  값을 바꾸면 약관 제12조의2와 /mypage/points 안내 문구도 함께 고칠 것. */
