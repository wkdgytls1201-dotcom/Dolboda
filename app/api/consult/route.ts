    ...
    try {
      await resend.emails.send({
        ...
        text:
          ...
          `전국 요양병원·요양원·주야간보호·방문요양 28,000여 곳의 정보를 한 화면에서 비교할 수 있게 ` +
          `정리한 서비스입니다. 국민건강보험공단과 건강보험심사평가원이 공개한 자료를 사용하며, ` +
          `평가등급·인력 현황·비급여 비용·프로그램 운영처럼 흩어져 있던 정보를 보호자가 한 번에 ` +
          `확인할 수 있게 합니다.\n` +
          `저희는 시설과 보호자 사이의 정보 비대칭을 줄이는 일을 합니다 — 발품과 입소문에 기대던 ` +
          `과정을, 공개된 데이터로 먼저 비교하고 방문할 곳을 좁히는 과정으로 바꾸는 것입니다. ` +
          `어느 시설을 선택할지는 보호자와 시설이 직접 상담해 정하는 일이며, ${SITE_NAME}는 그 사이에 ` +
          `개입하지 않습니다.\n\n` +
          `[정보가 실제와 다르다면 알려주세요]\n` +
          `표시되는 정보는 공공데이터 기준이라 갱신 시점 차이로 실제와 다를 수 있습니다. ` +
          `정정이 필요하면 이 메일에 회신해 주시면 확인 후 반영하겠습니다.\n\n` +
          `[문의 전달을 원하지 않으시면]\n` +
          `앞으로 상담 문의 전달을 받지 않으시려면 이 메일에 "수신거부"라고 회신해 주세요. ` +
          `이후 귀 시설로는 전달하지 않습니다.\n\n` +
