    ...
    try {
      await resend.emails.send({
        ...
        text:
          ...
          // ⚠️ lib/resend.ts의 HTML 본문과 같은 내용을 유지할 것 —
          // 광고 상품(/business)과 앞뒤가 맞아야 하는 문장이다.
          `어느 시설을 선택할지는 보호자와 시설이 직접 상담해 정하시며, ${SITE_NAME}는 그 사이에 ` +
          `개입하지 않습니다. 목록의 순서는 거리·평가등급·안심지수 기준으로 매겨지고, 별도로 ` +
          `운영하는 스폰서 노출은 일반 목록과 분리된 자리에 '광고'로 표시해 보호자가 바로 ` +
          `구분할 수 있게 합니다.\n\n` +
          `[혹시 정보가 실제와 다르다면]\n` +
