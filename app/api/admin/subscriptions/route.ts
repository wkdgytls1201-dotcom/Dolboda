...
export async function PATCH(req: Request) {
  ...
  if (TERMINAL.has(sub.status)) {
    return NextResponse.json(
      {
        error:
          "이미 해지된 구독이에요. 다시 이용하려면 새로 신청·승인해주세요.",
      },
      { status: 409 },
    );
  }
  ...
  //
  // 이 빠른 검사는 트랜잭션 밖이라 그 자체로는 동시 활성화 레이스를 못 막는다 —
  // 아래 트랜잭션 안에서 지역별 advisory lock을 걸고 한 번 더 확인한다.
  if (status === "active") {
    for (const p of placements) {
      const used = await heldSlots(
        p.scope as "sigungu" | "sido" | "national",
        p.regionKey,
        {
  ...
  try {
    updated = await prisma.$transaction(async (tx) => {
      if (status === "active") {
        for (const p of placements) {
          await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${"biz-slot:" + p.regionKey}))`;
          const used = await heldSlots(
            p.scope as "sigungu" | "sido" | "national",
            p.regionKey,
            { excludeFacilityId: sub.facilityId, onlyActive: true },
            tx,
          );
          if (used >= SPONSOR_SLOTS_PER_SIGUNGU) {
            throw new Error(`SLOT_FULL:${p.regionKey}`);
          }
        }
        if (banner) {
          await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${"biz-slot:" + banner.regionKey}))`;
          const used = await heldBannerSlots(
            banner.regionKey,
            { excludeFacilityId: sub.facilityId, onlyActive: true },
            tx,
          );
          if (used >= BANNER_SLOTS_PER_SIGUNGU) {
            throw new Error(`SLOT_FULL:${banner.regionKey}`);
          }
        }
      }
    ...
    if (e instanceof Error && e.message.startsWith("SLOT_FULL:")) {
      const region = e.message.slice("SLOT_FULL:".length);
      return NextResponse.json(
        {
          error: `${region}의 자리가 방금 다른 활성화로 다 찼어요. 다른 곳을 내리거나 대기로 안내해주세요.`,
        },
        { status: 409 },
      );
    }
