...
const STEP_TITLES = ["언제, 어디서", "어떤 상황이신가요", "원하시는 조건"];
...
function CareRequestForm({
  ...
  function canProceed() {
    if (step === 0) return region && startDate && endDate;
  ...
  return (
    <main className="mx-auto max-w-lg px-4 pb-28 pt-6">
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink-700">어떤 상황이신가요</label>
            <textarea
              value={situation}
              onChange={(e) => setSituation(e.target.value)}
              rows={3}
              placeholder="예: 수술 후 회복 중이라 거동을 도와줄 분이 필요해요"
