...
const HIDDEN_PREFIXES = ["/facility/", "/grade-test", "/care-request"];
      ...
      >
        <div className="grid grid-cols-4">
          {TABS.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`flex min-h-[56px] flex-col items-center justify-center gap-0.5 py-1.5 text-[11px] font-semibold transition-colors duration-150 ${
          ...
          {session?.user ? (
            ...
            >
              <User size={20} strokeWidth={myPageActive ? 2.4 : 2} />
              마이페이지
            </Link>
          ) : (
            <button
              ...
              className="flex min-h-[56px] flex-col items-center justify-center gap-0.5 py-1.5 text-[11px] font-semibold text-ink-400 transition-colors duration-150 active:text-primary-600"
            >
              <User size={20} />
              마이페이지
            </button>
